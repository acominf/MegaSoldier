...
public class FloorEntity extends Actor {
    ...
    public FloorEntity(World world, Texture floor, Texture overfloor, float x, float width, float y) {
        ...
        BodyDef leftDef = new BodyDef();                // (1) Provide some definition.
        leftDef.position.set(x, y - 0.55f);  // (2) Center the floor in the coordinates given
        leftBody = world.createBody(leftDef);               // (3) Create the floor. Easy.

        PolygonShape leftBox = new PolygonShape();      // (1) Create the polygon shape.
        leftBox.setAsBox(0.02f, 0.45f);              // (2) Give it some size.
        leftFixture = leftBody.createFixture(leftBox, 1);       // (3) Create a fixture.
        leftFixture.setUserData("cuadros");               // (4) Set the user data for the fixture.
        leftBox.dispose();                              // (5) Destroy the shape


        // Now place the actor in the stage by converting the coordinates given in meters to px.
        setSize(width * Constants.PIXEL_IN_METER, Constants.PIXEL_IN_METER);
        setPosition(x * Constants.PIXEL_IN_METER, (y - 1) * Constants.PIXEL_IN_METER);
    }
